...
## やったこと (新しい順)
- 2026-05-31: カレンダー機能実装。`src/lib/calendar.ts` に純粋関数 (`buildMonthGrid` / `expandSubscriptionsToMonth` / `groupEventsByDate` 等) を実装し Vitest 22件通過。`src/components/calendar/` 配下に4コンポーネント作成 (calendar.tsx / calendar-header.tsx / calendar-grid.tsx / calendar-cell.tsx)。`/calendar` ルート追加、祝日 (`@holiday-jp/holiday_jp`) 対応、土日カラーリング、サブスク請求日バッジ表示。Header のナビゲーションリンクを実パスに変更。
